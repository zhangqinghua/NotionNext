...
export default function ArticleDetail(props) {
  ...
  // 没有内容，屏蔽封面图，然后用封面图作为内容
  let noContent = post?.content?.length == 0;

  return (
    <div id="container" className="max-w-5xl overflow-x-auto flex-grow mx-auto w-screen md:w-full ">

      {/* 封面图 */}
      {!noContent && post?.type && !post?.type !== 'Page' && post?.pageCover && (
      ...
      >
        ...
        <section id='notion-article' className='px-1'>
          ...
          {/* 有内容，则直接展示内容。 */}
          {post && !noContent && <NotionPage post={post} />}
        </section>
