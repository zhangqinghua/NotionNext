...
async function getDataBaseInfoByNotionAPI({ pageId, from }) {
  console.log('========================================getDataBaseInfoByNotionAPI star t')
  // 3. 请求通告文章详情
  const originPages = await getAllPost(pageId)
  const collectionData = await getPageProperties(originPages)
  ...
  const categoryOptions = getAllCategories({ database, allPosts })
 ...
 */
/* eslint-disable no-unused-vars */
export async function getNotionPageData1({ pageId, from }) {
  ...
  try {
    ...
    // 1. 先从缓存里面读取
    data = await getDataFromCache(cacheKey)
    if (data && data.pageIds?.length > 0) {
      console.log('[缓存]:', `from:${from}`, `root-page-id:${pageId}`)
      return data
    }
...
export async function getNotionPageData({ pageId, from }) {
  ...
  console.log('没有命中缓存，返回空数据')
  const result = EmptyData(pageId)
...
/**
 * 从 Notion API 接口加载 Notion 数据并缓存
 */
export async function loadNotionPageDataAndCache({ pageId, from }) {
  // 1. 调用远程接口读取
  console.log('调用 Notion API 接口获取数据库信息和文章列表 start: ', pageId)
  const db = await getDataBaseInfoByNotionAPI({ pageId, from })
  console.log('调用 Notion API 接口获取数据库信息和文章列表 finish: ', pageId)
  // 2. 存入缓存
  if (db) {
    const cacheKey = 'page_block_' + pageId
    await setDataToCache(cacheKey, db, 30 * 24 * 60)
  }
  return 'success'
}
...
export async function getGlobalData({ pageId = BLOG.NOTION_PAGE_ID, from }) {
  console.log('getNotionData -> getGlobalData')
  // 从notion获取
