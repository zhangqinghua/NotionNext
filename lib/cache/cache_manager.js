...
// 使用文件缓存，因为内存缓存好像有并发Bug23435466gg
...
if (process.env.MONGO_DB_URL && process.env.MONGO_DB_NAME) {
  ...
  api = FileCache
} else if (process.env.ENABLE_REDIS_CACHE) {
  api = RedisCache
} else {
  api = MemoryCache
}
...
export async function setDataToCache(key, data, minutes) {
  if (!data) {
    return
  }
  await api.setCache(key, data, minutes)
}
